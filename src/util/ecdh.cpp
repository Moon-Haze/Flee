...
#define DEFAULTSHAREKEY                                                      \
    Flee::toByteArrayFromHex("C1 29 ED BA 73 6F 49 09 EC C4 AB 8E 01 0F 46 " \
                             "A3")

namespace Flee {

ByteArray ECDH::keyStr{ toByteArrayFromHex(
...
ECDH ECDH::generateKeyPair() {
    ...
    ByteArray privateKey(dh.PrivateKeyLength()),
        publicKey(dh.PublicKeyLength()), shareKey(dh.AgreedValueLength());

    dh.GenerateKeyPair(rng, ( unsigned char* )privateKey.data(),
                       ( unsigned char* )publicKey.data());

    dh.Agree(( unsigned char* )shareKey.data(),
...
bool ECDH::isDefault() const {
    return this->publicKey == DEFAULTPUBLICKEY
           && this->shareKey == DEFAULTSHAREKEY;
}
...
const ByteArray& ECDH::getShareKey() const { return shareKey; }

void ECDH::setPrivateKey(const ByteArray& value) { ECDH::privateKey = value; }

const ByteArray& ECDH::getPublicKey() const { return publicKey; }

void ECDH::setPublicKey(const ByteArray& value) { ECDH::publicKey = value; }

void ECDH::setShareKey(const ByteArray& value) { ECDH::shareKey = value; }

const ByteArray& ECDH::getPrivateKey() const { return privateKey; }

const ByteArray& ECDH::getMaskedShareKey() const { return maskedShareKey; }

void ECDH::setMaskedShareKey(const ByteArray& value) {
    ECDH::maskedShareKey = value;
}

const ByteArray& ECDH::getKeyStr() { return keyStr; }

void ECDH::setKeyStr(const ByteArray& value) { ECDH::keyStr = value; }
} // namespace Flee
